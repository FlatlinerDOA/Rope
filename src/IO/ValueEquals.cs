...
public record class ValueEquals(string Column, string Value) : Search()
{
	...
	public override IEnumerable<(long Start, long End)> SearchablePages(FileIndex index) => index.ColumnRanges[this.Column].Where(r => r.Filter.MightEqual(this.Value)).Select(r => (r.StartBytePosition, r.EndBytePosition));
