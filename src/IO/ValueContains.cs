...
public record class ValueContains(string Column, string Value) : Search()
{
	public override bool ShouldSearch(FileIndex index) => index.ColumnRanges.ContainsKey(this.Column);

	public override IEnumerable<(long Start, long End, int StartRowIndex)> SearchablePages(FileIndex index) => index.ColumnRanges[this.Column].Where(r => r.Filter.MightContain(this.Value)).Select(r => (r.StartBytePosition, r.EndBytePosition, r.StartRowIndex));
