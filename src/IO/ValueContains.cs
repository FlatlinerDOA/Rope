...
public record class ValueContains(string Column, string Value) : Search()
{
	...
	public override IEnumerable<(long Start, long End)> SearchablePages(FileIndex index) => index.ColumnRanges[this.Column].Where(r => r.Filter.MightContain(this.Value)).Select(r => (r.StartBytePosition, r.EndBytePosition));
